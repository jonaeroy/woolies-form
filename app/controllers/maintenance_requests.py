...
class MaintenanceRequests(Controller):
    class Meta:
        prefixes = ('admin',)
        components = (scaffold.Scaffolding, SplitView, Pagination, Upload, Drafts)
        pagination_limit = 10
        sv_result_variable = 'maintenance_requests'
    ...
    @route_with(template='/maintenance_requests/page/add')
    def add_form(self):
        ...
        def after_save(controller, container, item):
            ...
            subject = "DC Maintenance Request Notification"
            ...
            # Send an email to the assiged To field and CC'd address
            to = item.to
        ...
        address_list = User.order_by_fullname_asc()
        # dc_warehouse_info = Dc.get_dcs()
        dc_warehouse_info = DcWarehouse.get_all()

        self.context['address_list'] = address_list
        self.context['dc_warehouse_info'] = dc_warehouse_info

        self.meta.view.template_name = 'maintenance_requests/form.html'

        self.context['PAGE_TITLE'] = 'DC Maintenance Request Form'
        self.context['domain_path'] = self.session.get('DOMAIN_PATH')
        self.context['user_fullname'] = self.session.get('user_fullname')

        self.scaffold.redirect = '/maintenance_requests?key=' + form_key

        return scaffold.add(self)

    @route_with(template='/maintenance_requests/retrieve/<dc_name>')
    def get_dc_email_address(self, dc_name):
        result = DcWarehouse.find_all_by_dc_name(dc_name).get()

        if result.email_address is not None:
    ...
    def view(self, key):
        ...
        #self.context['warehouse_location'] = result.warehouse_location
        self.context['requestor_name'] = result.requestor_name
        self.context['date_time'] = result.date_time
        self.context['department'] = result.department
        self.context['notes'] = result.notes
        self.context['priority'] = result.priority
        ...
        self.context['PAGE_TITLE'] = 'DC Maintenance Request View'
    ...
    def list(self):
        ...
        if self.request.get('order_by_created'):
            order = self.request.get('order_by_created') == 'desc' and MaintenanceRequest.created_by or -MaintenanceRequest.created_by
        elif self.request.get('order_by_status'):
            order = self.request.get('order_by_status') == 'desc' and MaintenanceRequest.status or -MaintenanceRequest.status
        else:
            order = self.request.get('order_by_date') == 'desc' and MaintenanceRequest.created or -MaintenanceRequest.created

        if showAll:
            self.context['maintenance_requests'] = MaintenanceRequest.query().order(order)
        else:
            self.context['maintenance_requests'] = MaintenanceRequest.query(MaintenanceRequest.created_by == users.get_current_user()).order(order)

        self.context['PAGE_TITLE'] = 'DC Maintenance Request List'
        ...
        self.context['sv_maintenance_requests'] = self.context['maintenance_requests']

    @route_with(template='/maintenance_requests/page/edit/<key>/<form_key>')
    def edit_data(self, key, form_key):
        ...
        self.meta.view.template_name = 'maintenance_requests/update.html'
        self.scaffold.redirect = '/maintenance_requests?key=' + form_key
        ...
        def after_save(controller, container, item):
            status = 'Edited'
            additional_comments = 'N/A'
            subject = "Change in DC Maintenance Request Notification"
            to = Users.get_user_list_by_group(controller.context.get('first_group_approver').key.urlsafe())
            ...
            # Send an email to assigned To field and CC'd address
            to = item.to
            self.sendMail(status, item, additional_comments, to, subject, domain_path, cc, encoded_param)
    ...
    @route_with(template='/maintenance_requests/fetch_request_status/<key>')
    def fetch_status(self, key):
        result = self.util.decode_key(key).get()
        return str(result.status)

    @route_with(template='/maintenance_requests/remote_process/<entity_key>/<form_key>/<flag>')
    def approve_reject_via_email(self, entity_key, form_key, flag):
        ...
        if user in _list:
            ...
            encoded_param = "?key=" + form_key
            additional_comments = 'N/A'
            subject = "DC Maintenance Request Approval Notification"
            ...
            if item:
                if flag == 'tempapprove' and item.status == 1:
                    ...
                    status = 4
                    exp_current_status = 1
                    subject = "DC Maintenance Request Rejection Notification"
                ...
                if item.status == exp_current_status:
                    ...
                    if flag == 'reject_level_1' or flag == 'reject_level_2' or flag == 'approve':
                        ...
                        # Send and email to the submitter, to and cc'd address
                        to = item.to + ";" + str(item.created_by)
                        self.sendMail(form_status, item, additional_comments, to, subject, domain_path, cc, encoded_param)
                else:
    ...
    def sendMail(self, form_status, resource, additional_comments, to, subject, domain_path, cc, encoded_param, approve_link='#', reject_link='#'):
        ...
        dc = resource.dc
        # warehouse_location = resource.warehouse_location
        ...
        msg_body = """\
                ...
                <table style="padding: 0; margin: 0; width:600px; font-family:Arial,Helvetica,sans-serif; font-size: 12px; letter-spacing: 1px; border-spacing: 0; background-color: #fff;" border="0">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td colspan="2" style="padding: 0; margin: 0; width: 500px; height: 150px; background: #2c3742; color: #fff; text-align: justify;">
                                <p>Hi, <br/><br/><br/>The request for DC maintenance has been <span style="color: #ff502d"><strong>%s</strong></span> on <span style="color: #ff502d"><strong>%s</strong></span> by <span style="color: #ff502d"><strong>%s</strong></span> with the following details:</p>
                            </td>
                            <td style="padding: 0; margin: 0; width: 50px; height: 150px; background: #2c3742;">&nbsp;</td>
                        </tr>
                        <!-- opening -->
                        ...
                        <tr>
                            ...
                            <td style="padding: 15px 0 7px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                        ...
                        <tr>
                            ...
                            <td style="padding: 7px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-size: 11px; color: #7d7d7d; font-weight: bold; text-transform: uppercase;">To</span>
                            </td>
                            <td style="padding: 7px 0; margin: 0; width: 80px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-weight: bold; color: #009900;">%s</span>
                            </td>
                        ...
                        <tr>
                            <td style="padding: 0; margin: 0; width: 50px;">&nbsp;</td>
                            <td style="padding: 7px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-size: 11px; color: #7d7d7d; font-weight: bold; text-transform: uppercase;">CC</span>
                            </td>
                            <td style="padding: 7px 0; margin: 0; width: 80px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-weight: bold; color: #009900;">%s</span>
                            </td>
                        ...
                        <tr>
                            ...
                            <td colspan="2" style="padding: 15px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-size: 11px; color: #7d7d7d; font-weight: bold; text-transform: uppercase;">Name of Requestor</span><br/>
                        ...
                        <tr>
                            <td style="padding: 0; margin: 0; width: 50px;">&nbsp;</td>
                            <td style="padding: 7px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-size: 11px; color: #7d7d7d; font-weight: bold; text-transform: uppercase;">Priority</span>
                            </td>
                            <td style="padding: 7px 0; margin: 0; width: 80px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-weight: bold; color: #009900;">%s</span>
                            </td>
                        ...
                        <tr>
                            <td style="padding: 0; margin: 0; width: 50px;">&nbsp;</td>
                            <td style="padding: 7px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-size: 11px; color: #7d7d7d; font-weight: bold; text-transform: uppercase;">Date/Time</span>
                            </td>
                            <td style="padding: 7px 0; margin: 0; width: 80px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-weight: bold; color: #009900;">%s</span>
                            </td>
                        ...
                        <tr>
                            ...
                            <td colspan="2" style="padding: 20px 0; margin: 0; width: 420px; border-bottom: 1px solid #d8dee3;">
                                <span style="font-size: 11px; color: #7d7d7d; font-weight: bold; text-transform: uppercase;">Description of Fault/Defect/Task/Equipment</span><br/>
                         ...
                         <tr>
                            ...
                            <td colspan="2" style="padding: 0; margin: 0; width: 500px; height: 100px;">
                                <p style="color: #909090; font-size: 12px; text-align: left;">Click here to view list<br/>
                                    <a style='color: #3b9ff3;' target='_blank' href='http://%s/maintenance_requests%s'>http://%s/maintenance_requests%s</a>
                                </p>
                            </td>
                            <td style="padding: 0; margin: 0; width: 50px; height: 50px;"></td>
                        </tr>
                   ...
                   """ % (form_status, date_time, user, dc, to, display_cc,
                            requestor_name, priority, date_time, department, notes,
                            attachment, additional_comments,
                            approve_reject_body,
